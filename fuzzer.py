...
def controlled_manipulation_fuzzer(image, blur_factor, contrast_factor, brightness_factor):
...
def run_model(img_data):
    ...
    return detection_classes, num_detections

def post_process(img, num_detections, detection_boxes, detection_classes):
    # Post processing
        coco_classes = {
            0: u'__background__',
            1: u'person',
...
def main():
    ...
    raw_image = Image.open("test_images/traffic-light.jpg")
    detection_classes_raw, num_detections_raw = run_model(pre_process(raw_image))
    

    #Demo of how fuzzing would work and score (testing brightness only)
    for i in range(5):
        print("-------RUN WITH BRIGHT FACTOR = " + str(i))
        fuzzed_image = controlled_manipulation_fuzzer(raw_image, i, i, i)
        detection_classes, num_detections = run_model(pre_process(fuzzed_image))
        if (detection_classes.all() == detection_classes_raw.all()):
            print("test passed")
